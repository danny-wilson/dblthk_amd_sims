...
setClass("MRBF2",
         representation(Exposure = "character",
						...
						BMAve_StdError = "numeric",
						BestModel_Estimate = "numeric",
						BestModel_StdError = "numeric",
						BestModel = "character",
						tupel = "character",
						pp="numeric",
						pp_marginal="numeric",
...
summary_mvMR_BF = function(object, sigma=0.5, prior_prob=0.5, calc.se=FALSE){
	...
	if(calc.se) theta2_all = matrix(numeric(0), (ncol(bX)),0)
	...
	#for (i in 1:5){
		...
		if(calc.se) {
			theta_est = array(apply(combi, MARGIN=2, FUN = beta2_summary, XtY=XtY, XtX=XtX, YtY=YtY, n=n_SNPs, sigma_vec=sigma_vec), c(n_RF, 2, ncol(combi)))
			theta_all = cbind(theta_all, theta_est[,1,])
			theta2_all = cbind(theta2_all, theta_est[,2,])
		} else {
	...
	if(calc.se) {
		BMA_estimate2 = rowSums(pp_mat * theta2_all)
		bma_stderr = sqrt(BMA_estimate2-BMA_estimate^2)
		best_model_stderr = sqrt(theta2_all[,which.max(log_evidence)]-best_model_est^2)
	}
	#marginal inclusion
	...
	if(calc.se) {
		return(new("MRBF2",
			...
			BMAve_StdError = bma_stderr,
			BestModel_Estimate = best_model_coef,
			BestModel_StdError = best_model_stderr,
...
#
# compute the mean and second moment in effect size for a set of risk factors given gamma (indicator vector)
#


beta2_summary = function(XtY=XtY, XtX=XtX, YtY=YtY, n, sigma_vec, gamma){
	...
	invnu=diag(sigma_g^-2, ncol=length(gamma))
	theta_out = matrix(0.0, nrow(XtY), 2)
	colnames(theta_out) <- c("E[theta]", "E[theta^2]")
	...
	tau.alpha = 0.5*n
	tau.beta = 0.5*as.vector(YtY - t(B) %*% invOmega %*% B)
	theta_out[gamma,2] = diag(tau.alpha/(tau.beta-1) * Omega + B %*% t(B))
